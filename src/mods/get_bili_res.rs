...
use super::types::{BiliConfig, RequestPlayurl, ResignInfo};
...
pub async fn get_playurl(req: &HttpRequest, is_app: bool, is_th: bool) -> impl Responder {
    let (pool, config, tx) = req
        .app_data::<(Pool, BiliConfig, Sender<RequestPlayurl>)>()
        .unwrap();
    ...
    match redis_get(&pool, &key).await {
        Some(value) => {
            if &value[..13].parse::<u64>().unwrap() > &ts {
                ...
                is_expire = false;
            } else if &value[..13].parse::<u64>().unwrap() + 1200000 > ts {
                redis_get_data = value[13..].to_string();
                is_expire = false;
                let send = RequestPlayurl {
                    redis: pool,
                    user_agent: user_agent.clone(),
                    access_key: access_key.clone(),
                    build: query.get("build").unwrap_or("6800300").to_string(),
                    device: query.get("device").unwrap_or("android").to_string(),
    ...
    if is_expire {
            ...
            .get(&body_data_json["code"].as_i64().unwrap().to_string())
        {
            ...
            None => config.cache.get("other").unwrap().clone(),
        };
...
async fn get_playurl_nearexpire(request_playurl: RequestPlayurl<'_>) {
    let dt = Local::now();
    let ts = dt.timestamp_millis() as u64;
    let ts_string = ts.to_string();
    let mut query_vec = vec![
        ("access_key", request_playurl.access_key),
        ("appkey", request_playurl.appkey.clone()),
        ("build", request_playurl.build),
        ("device", request_playurl.device),
    ...
    match request_playurl.ep_id {
        Some(value) => query_vec.push(("ep_id", value)),
        None => (),
    }
    match request_playurl.cid {
        Some(value) => query_vec.push(("cid", value)),
        None => (),
    }
    let appkey: &str;
    let appsec: String;
    match request_playurl.area_num {
        4 => {
            appkey = "7d089525d3611b1c";
            appsec = appkey_to_sec(&appkey).unwrap();
            query_vec.push(("s_locale", "zh_SG".to_string()));
        }
        _ => {
            appkey = &request_playurl.appkey;
            appsec = appkey_to_sec(&request_playurl.appkey).unwrap();
        }
    }
    query_vec.sort_by_key(|v| v.0);
    let unsigned_url = qstring::QString::new(query_vec);
    ...
    let proxy_open = match request_playurl.area_num {
        1 => &request_playurl.config.cn_proxy_playurl_open,
        2 => &request_playurl.config.hk_proxy_playurl_open,
        3 => &request_playurl.config.tw_proxy_playurl_open,
        4 => &request_playurl.config.th_proxy_playurl_open,
        _ => &request_playurl.config.tw_proxy_playurl_open,
    };
    let proxy_url = match request_playurl.area_num {
        1 => &request_playurl.config.cn_proxy_playurl_url,
        2 => &request_playurl.config.hk_proxy_playurl_url,
        3 => &request_playurl.config.tw_proxy_playurl_url,
        4 => &request_playurl.config.th_proxy_playurl_url,
        _ => &request_playurl.config.tw_proxy_playurl_url,
    };
    let api = match request_playurl.is_app {
        true => match request_playurl.area_num {
            1 => &request_playurl.config.cn_app_playurl_api,
            2 => &request_playurl.config.hk_app_playurl_api,
            3 => &request_playurl.config.tw_app_playurl_api,
            4 => &request_playurl.config.th_app_playurl_api,
            _ => &request_playurl.config.tw_app_playurl_api,
        },
        false => match request_playurl.area_num {
            1 => &request_playurl.config.cn_web_playurl_api,
            2 => &request_playurl.config.hk_web_playurl_api,
            3 => &request_playurl.config.tw_web_playurl_api,
            4 => &request_playurl.config.th_web_playurl_api,
            _ => &request_playurl.config.tw_web_playurl_api,
        },
    };
    ...
    let body_data_json: serde_json::Value = serde_json::from_str(&body_data).unwrap();
    let expire_time = match request_playurl
        .config
        .cache
        .get(&body_data_json["code"].as_i64().unwrap().to_string())
    {
        Some(value) => value.clone(),
        None => request_playurl.config.cache.get("other").unwrap().clone(),
    };
    let value = format!("{}{body_data}", ts + expire_time * 1000);
    let _: () = redis_set(
        request_playurl.redis,
        &request_playurl.key,
        &value,
        expire_time,
    )
...
pub async fn get_season(req: &'_ HttpRequest, _is_app: bool, _is_th: bool) -> impl Responder {
    let (pool, config) = req.app_data::<(Pool, BiliConfig)>().unwrap();
    match req.headers().get("user-agent") {
        Option::Some(_ua) => (),
        _ => {
